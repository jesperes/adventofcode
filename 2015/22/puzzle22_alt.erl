...
print_state(S) ->
    io:format("Turn: hp = ~10w, mana = ~10w, boss_hp = ~10w, effects = ~w~n",
              [maps:get(hp, S),
               maps:get(mana, S),
               maps:get(boss_hp, S),
               maps:get(effects, S)]).
            ...
            Winner = 
                lists:foldl(
                  fun(S, undef) ->
                          ...
                          %% and battle
                          io:format("Recursing into battle (1): ~w~n", [S]),
                          CandidateWinner = battle(boss, S),
                          [#{winner := CandWin} = CandHead|_CandRest] = CandidateWinner,
                          case CandWin of
                              true ->
                                  io:format("Got first winner: ~w~n", [CandHead]),
                                  CandidateWinner;
                              false ->
                                  io:format("Pruning, not a winner: ~w~n", [CandHead]),
                                  undef
                          end;
                     ...
                     (S, [WinnerHead|_] = CurrentWinner) ->
                          ...
                          if CurrentBest < CurrentlySpent ->
                                  ...
                                  io:format("Pruning (overspending 1): ~w~n", [S]),
                                  CurrentWinner;
                             true ->
                                  ...
                                  %% Recurse into battle!
                                  io:format("Recursing into battle (2): ~w~n", [S]),
                                  ...
                                  case {CandWin, LowestScore} of
                                      {player, Score} when Score < CurrentBest ->
                                          io:format("Improved score = ~w~n", [Score]),
                                          [CandHead#{lowest_mana_spent => Score}|CandRest];
                                      _ ->
                                          io:format("Pruning (overspending 2) ~w~n", [CandHead]),
                                          CurrentWinner
                                  end
                          end
                  end, 
...
battle(boss, State) ->
    ...
    case maps:get(boss_hp, S1) of
        ...
        _ ->
            ...
            %% erlang:display({boss_damage, BossDamage}),
