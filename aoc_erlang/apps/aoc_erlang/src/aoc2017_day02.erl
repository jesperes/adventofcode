...
part1(Input) ->
  lists:foldl(fun(Line, Acc) ->
                  Acc + abs(lists:min(Line) - lists:max(Line))
              end, 0, Input).
...
part2(Input) ->
  lists:foldl(
    fun(Line, Acc) ->
        Acc + lists:sum(
                [A div B || A <- Line, B <- Line,
                            A > B, A rem B == 0])
    end, 0, Input).

example_test_() ->
  [ ?_assertEqual(18, part1(?TEST_INPUT))
  , ?_assertEqual(9, part2(?TEST_INPUT))
  ].

main_test_() ->
  [ {"Part 1", ?_assertEqual(37923, part1(?INPUT))}
