...
main_test_() ->
  %% For part 1, we don't have the answer as a string, but as a pixel
  %% array, see print_display/1 below.
  {"Part 2", ?_assertEqual(10036, start1())}.
...
input() ->
  Str = inputs:get_as_string(2018, 10),
  Points =
    lists:map(fun(Line) ->
