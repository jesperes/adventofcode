...
parse_line(Line) ->
  ...
  %% "viable pairs", i.e. pairs of nodes where you can move data
  %% from A to B. This was trivial (see find_viable_pairs/1 below).
  %%
...
start(Nodes) ->
  SortedNodes =
    lists:sort(fun(#{pos := {Xa, Ya}}, #{pos := {Xb,Yb}}) ->
                   {Ya,Xa} =< {Yb,Xb}
               end, Nodes),
...
find_viable_pairs(Nodes) ->
  lists:filter(fun({#{used := A_used}, #{avail := B_avail}}) ->
               ...
               end, [{P1, P2} || P1 <- Nodes, P2 <- Nodes, P1 /= P2]).

main_test_() ->
  Binary = inputs:get_as_binary(2016, 22),
  Nodes = parse(Binary),
  [ {"Part 1",
     ?_assertEqual(864, length(find_viable_pairs(Nodes)))}
  , {"Part 2",
     %% Part 2 was solved manually by looking at the plotted grid, so
     %% this test doesn't really do anything except create the plotted
     %% grid.
     ?_assertEqual(ok, start(Nodes))}
  ].
