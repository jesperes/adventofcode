...
main_test_() ->
  Input = inputs:get_as_binary(2018, 17),
  Veins = veins(Input),
  Grid = grid_new(Veins),
  {MinY, MaxY} = find_y_bounds(Veins),
  {_, G0} = flow_down(Grid, starting_point(), MaxY),
  [ {"Part 1", ?_assertEqual(34775, count_reachable_tiles(G0, MinY, MaxY))}
  , {"Part 2", ?_assertEqual(27086, count_tiles_after_draining(G0, MinY, MaxY))}
  ].
