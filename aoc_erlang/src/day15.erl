...
-spec coords(integer(),integer()) -> coord().
coords(Width, Offset) ->
  Y = Offset div Width,
  X = Offset rem Width,
  {Y, X}.

%% Creates a key-value map of positions -> HP (hit points).
-spec units(binary(),_,binary()) -> gb_trees:tree(Pos :: coord(), HP :: integer()).
...
-type search_node() ::
        { Cost :: integer(),       %% Estimated cost to target
          FoePos :: coord(),       %% Current position during search
          PrevPos :: coord(),      %% This is the position which we ultimately move to
          TargetPos :: coord() }.  %% This is the "enemy adjacent" position ("in range" in the puzzle description)

-spec move(TargetPos :: coord(),
           Game :: any(),
           Foes :: gb_trees:tree(Pos :: coord(), HP :: integer())) -> move_result().
...
-spec move(Heads :: gb_sets:set(search_node()),
           Visited :: gb_sets:set(_),
           TargetPos :: coord(),
           Game :: any()) ->
        move_result().
