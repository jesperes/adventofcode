...
public class AocBaseRunner {
	public static void run(IAocPuzzle<?, ?, ?> puzzle) {
		...
		try {
			...
			if (info.hasInputFile()) {
				File inputFile = new File(String.format("inputs/%d/input%02d.txt", info.year(), info.day()));
				if (!inputFile.exists()) {
	...
	private static <T, P1, P2> void runWithInput(Optional<File> file, IAocPuzzle<T, P1, P2> puzzle) throws IOException {
		...
		System.out.println(expected.p1().get().equals(p1) ? String.format("OK (%s)", p1)
				: String.format("FAILED (got %s, expected %s)", p1, expected.p1().get()));
		...
		if (puzzle.getInfo().day() == 25) {
			...
			System.out.println(expected.p2().get().equals(p2) ? String.format("OK (%s)", p2)
					: String.format("FAILED (got %s, expected %s)", p2, expected.p2().get()));
		}
	}
}
