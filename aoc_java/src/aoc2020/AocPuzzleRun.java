...
public record AocPuzzleRun<T, P1, P2> (IAocPuzzle<T, P1, P2> puzzle, AocResult<P1, P2> result) {
	...
	public Map<AocResultTableField, String> toTableRow() {
		...
		if (timing.isPresent()) {
			if (info.hasInputFile)
				map.put(AocResultTableField.Parsing, timeToStr(timing.get().parsing()));
			else
				map.put(AocResultTableField.Parsing, "-");

			map.put(AocResultTableField.Part1Time, timeToStr(timing.get().part1()));
			map.put(AocResultTableField.Part2Time, timeToStr(timing.get().part2()));
		}

		if (result.p1().isPresent()) {
			map.put(AocResultTableField.Part1Result, result.p1().get().toString());
			map.put(AocResultTableField.Part1Status, statusBooleanToString(result.p1().equals(expected.p1())));
		}

		if (result.p2().isPresent()) {
			map.put(AocResultTableField.Part2Result, result.p2().get().toString());
			map.put(AocResultTableField.Part2Status, statusBooleanToString(result.p2().equals(expected.p2())));
		}
