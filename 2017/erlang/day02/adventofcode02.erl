-module(adventofcode02).
-include_lib("eunit/include/eunit.hrl").

-export([spreadsheet/0, spreadsheet/1, find_divisors/1]).
...
sum(List) ->
    lists:foldr(fun(Elem, Sum) ->
			Elem + Sum
		end, 0, List).

%%% Find the two elements in the list which divide evenly.
find_divisors(List) ->
...
spreadsheet() ->
    List = string_to_list_of_lists(?TEST_INPUT),    
    lists:foldr(fun({X,Y}, Sum) -> Sum + (X / Y) end, 0,
