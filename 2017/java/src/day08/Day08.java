...
public class Day08 {
	...
	int interpret(String input) {
		...
		for (String line : input.split("[\\r\\n]")) {
			...
			if (condition) {
				...
				// Read the previous value of the variable we are going to change.
		...
		System.out.format("Largest variable is %s = %d%n", maxVariable, maxValue);
		return maxValue;
	}
	...
	public void testPart1_full() throws Exception {
		try (BufferedReader inputReader = new BufferedReader(
				new InputStreamReader(getClass().getClassLoader().getResourceAsStream("day08/input.txt")))) {

			System.out.println(
					"[Day08]: Largest value: " + interpret(inputReader.lines().collect(Collectors.joining("\n"))));
		}
	}
	...
	public void testPart2_full() throws Exception {
		try (BufferedReader inputReader = new BufferedReader(
			...
			System.out.println("[Day08]: Largest value (part 2): "
					+ interpret_part2(inputReader.lines().collect(Collectors.joining("\n"))));
		}
	}
}
