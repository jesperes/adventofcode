...
public class Day13 {
    ...
    private Map<Integer, Firewall> getLongInput() throws IOException {
        return Utils.readFileFromClassPathAsLines(getClass(), "day13/input.txt")
                .map(s -> new Firewall(s))
                .collect(Collectors.toMap(fw -> fw.depth, Function.identity()));
    }
     ...
     * 
     * @param firewalls
     *            The firewalls. The index here is the depth of the firewalls,
     *            so if there are no firewall at a certain index, the array has
     *            a null element.
     * @param maxDepth
     ...
     *            If true, only collisions will be reported, and not the
     *            severity. This allows the function to return immediately when
